...
userSave(){
  ...
  this.http.post('http://microwebservice.net/ecodation/webService.php',body).subscribe(data=>{
        ...
        if(data==0){
         ...
         console.log("222222");
         this.modalClose();// kayıt olma işlemi olunca modal kendiliginden kapansın
         console.log("3333");
        }
...
git(){
  this.route.navigate(['home',this.ad]);
}
userSave(){
  console.log("Ad:"+this.ad);
  let body = {serviceName:'singUp',ad:this.ad,soyad:this.soyad,email:this.username,sifre:this.password}
  this.http.post('http://microwebservice.net/ecodation/webService.php',body).subscribe(data=>{
        ...
        if(data==0){
          ...
          console.log("Kayit oldu");
          this.sonuc = data[0];
          this.presentToast('success','Başarıyla Kayit oldunuz');
          localStorage.setItem('userJSON',JSON.stringify(data[0]));
         // this.userData = JSON.parse(localStorage.getItem('userJSON'));
          //let veri:12;
         // this.route.navigate(['home',veri]); // 'home' adresine git veri parametresi yolla 
         console.log("1. Hamle");
         this.modalClose(); //modal kapatma fonksiyonunu çagırıyoruz
         console.log("2.Hamle");
        }
